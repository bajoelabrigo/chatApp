...
- **Navegación estilo Facebook en la web (2026-08-26)**: `frontend/src/lib/useSwipeNav.js` (teclado, rueda y deslizar con el dedo) lo usan `StoryViewer` (eje x) y `ReelsPage` (eje y). El gesto exige que el **eje dominante** coincida —si no, un deslizamiento en diagonal salta de historia— y bloquea la rueda 500 ms, porque un solo gesto de trackpad emite decenas de eventos; se desactiva con un modal abierto (comentarios, "quién la vio") o las flechas del teclado navegarían mientras se escribe. **Las flechas visibles son `md:` solamente**: en el móvil se desliza y unos botones ahí taparían el video. Los carruseles usan `components/reels/CarouselRow.jsx`, que mide con `ResizeObserver` (el contenido llega por consulta, no basta medir al montar) y solo enciende la flecha del lado que tiene recorrido. Las dos pestañas (Historias y Reels) llevan delante los mismos dos accesos, "Reels" (a `/reels`, todos a pantalla completa) y "Crear" (modal), y viven en `components/reels/MediaShortcuts.jsx` para que se vean idénticos. **Son la CABECERA de la fila —una línea de texto sobre el carrusel—, nunca tarjetas dentro de él** (2026-09-05): hasta esa fecha eran dos tarjetas 9/16 al principio del carrusel y **en un teléfono solo caben ~3 tarjetas a la vez**, así que los accesos se comían la pantalla y dejaban ver UNA sola historia o UN solo reel — el contenido perdía contra sus propios accesos. El `ReelsStrip` intercalado cada 3 publicaciones NO lleva la cabecera de accesos —ahí es contenido de paso, solo el título y "Ver todos"—: la diferencia la marca recibir o no `onCreate`.
